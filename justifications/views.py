from django.shortcuts import render
from .models import Justification
from .serializers import JustificationSerializer
from rest_framework.decorators import api_view
from rest_framework.response import Response
from rest_framework import status

# Create your views here.
@api_view(['GET', 'POST'])
def justification_list(request):
    if request.method == 'GET':
        data = Justification.objects.all()
        serializer = JustificationSerializer(data, many=True)
        return Response(serializer.data)
    
    elif request.method == 'POST':
        serializer = JustificationSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

@api_view(['GET', 'PUT', 'DELETE'])
def justification_detail(request, id, format=None):
    try:
        justification = Justification.objects.get(pk=id)
    except Justification.DoesNotExist:
        return Response(status=status.HTTP_404_NOT_FOUND)

    if request.method == 'GET':
        serializer = JustificationSerializer(justification)
        return Response(serializer.data)
    
    elif request.method == 'PUT':
        serializer = JustificationSerializer(justification, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    
    elif request.method == 'DELETE':
        justification.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)